...
function itemLabel(item) {
    const name = String(item?.itemName || "Item Master item").trim();
    const ml = compactValue(item?.ml || item?.measureMl);
...
function mappedItemMarkup(item) {
    ...
    const ml = compactValue(mapped.ml || mapped.measureMl);
    return `<span class="mapped-choice done"><strong>${escapeHtml(ml ? `${ml} ML` : "ML -")}</strong><span>${escapeHtml(mapped.itemName || "Mapped item")}</span></span>`;
}
    ...
    container.innerHTML = entries.map((entry) => {
        ...
        return `
            <button class="candidate" data-code="${item.itemCode}" data-score="${entry.score || 0}" type="button">
                <span><strong>${escapeHtml(item.itemName || "Item Master item")}</strong><small>ML ${item.ml || "-"} | Pack ${item.packing || "-"}</small></span>
                ${score}
            </button>`;
    }).join("");
